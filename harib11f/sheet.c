...
struct SHTCTL *shtctl_init(struct MEMMAN *memman,
	unsigned char *vram, unsigned char *vbuf, int xsize, int ysize)
{
	struct SHTCTL *ctl;
	int i;
...
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1)
{
	...
	// finally flush vbuf to vram
	int offset;
	for (vy=vy0; vy < vy1; vy++) {
		for (vx = vx0; vx < vx1; vx++) {
			offset = vy * ctl->xsize + vx;
			vram[offset] = vbuf[offset];
		}
	}
	return;
}
